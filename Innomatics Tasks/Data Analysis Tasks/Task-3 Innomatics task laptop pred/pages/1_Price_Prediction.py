...
#Page Heading
st.header(":green[Laptop Price Prediction]")
...
with col1:
    brand = st.selectbox(
        ':red[1.Select Laptop Brand]',
        (df.Brand.unique()))
    st.write(brand)

with col2:
    operating_system=st.selectbox(
        ':red[2.Select Operating System]',
        (df['Operating System'].unique()))
    st.write(operating_system)

with col3:
    processor=st.selectbox(
        ':red[3.Select Processor Type]',
        (df['Processor'].unique()))
    st.write(processor)
...
with col1:
    ram_type=st.selectbox(
        ':red[4.Select RAM Type]',
        (df['RAM Type'].unique()))
    st.write(ram_type)

with col2:
    ram_size=st.selectbox(
        ':red[5.Select RAM Size]',
        (df['RAM Size'].unique()))
    st.write(ram_size)
...
with col1:
    disc_type=st.selectbox(
        ':red[6.Select DISC Type]',
        (df['Disc Type'].unique()))
    st.write(disc_type)

with col2:
    disc_size=st.selectbox(
        ':red[7.Select DISC Size]',
        (df['Disc Size'].unique()))
    st.write(disc_size)  
